...
import org.powerpoint.window.service.AbstractService;
...
public abstract class AbstractWindow <R extends AbstractService> extends JFrame {
    ...
    protected AbstractWindow(String title, Dimension defaultSize, boolean resizeable, Class<R> clazz){
        ...
        // 监听器，这里我们将窗口设计成点击'X'不直接关闭，由子类来决定是直接关闭还是做一些其他的事情
     ...
     * @param innerWidth 内部宽度（如窗口宽度）
     * @param innerHeight 内部高度（如窗口高度）
     * @return 居中位置
     */
    ...
    /**
     * 设定窗口的关闭动作，默认是调用 dispose() 方法，也可以设定为 System.exit()
     */
